...
void dialog_saveas()
{
    ...
    GetSaveFileName(&ofn);
    action_file_saveas(szFileName);
}
void dialog_save()
{
    TCHAR* buf;
    DWORD lsize = GetWindowTextLength(g.hwndEdit);
    DWORD dwTotal = (lsize + 1) * sizeof(TCHAR);
    buf = (TCHAR*)malloc(dwTotal);
    GetWindowText(g.hwndEdit, buf, dwTotal);
    HKEY hk;
    ...
    result = RegSetValueEx(hk, _T("Path"), NULL, REG_EXPAND_SZ, (BYTE*)buf, dwTotal);
    if(result != ERROR_SUCCESS) {
	Error2Msgbox(result, g.hwnd);
    } else {
